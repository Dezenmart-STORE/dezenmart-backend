...
import { TESTTOKENMINT } from './solana';
...
export class DezenMartContractService {
  ...
  private async writeContract(data: IwriteContract) {
    let { functionName, args = [], chain = CHAINENUMS.ethereum } = data;
  ...
  async registerLogisticsProvider(providerAddress: Address): Promise<Hash> {
    return await this.writeContract({
      functionName: 'registerLogisticsProvider',
      args: [providerAddress],
    });
  }

  async registerBuyer(): Promise<Hash> {
    return await this.writeContract({ functionName: 'registerBuyer' });
  }

  async registerSeller(chain?: CHAINENUMS): Promise<Hash> {
    return await this.writeContract({ functionName: 'registerSeller', chain });
  }
  ...
  async confirmDeliveryAndPurchase(purchaseId: string): Promise<Hash> {
    return await this.writeContract({
      functionName: 'confirmDeliveryAndPurchase',
      args: [purchaseId],
    });
  }
  ...
  async withdrawEscrowFees(): Promise<Hash> {
    return await this.writeContract({ functionName: 'withdrawEscrowFees' });
  }
