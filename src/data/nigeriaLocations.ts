export interface INigeriaState {
  state: string;
  lgas: string[];
}

export const nigeriaLocations: INigeriaState[] = [
  {
    state: 'Abia',
  ...
  {
    state: 'Akwa Ibom',
    lgas: ['Abak', 'Eastern Obolo', 'Eket', 'Esit Eket', 'Essien Udim', 'Etim Ekpo', 'Etinan', 'Ibeno', 'Ibesikpo Asutan', 'Ibiono-Ibom', 'Ini', 'Itu', 'Mbo', 'Mkpat-Enin', 'Nsit-Atai', 'Nsit-Ibom', 'Nsit-Ubium', 'Obot Akara', 'Okobo', 'Onna', 'Oron', 'Oruk Anam', 'Udung-Uko', 'Ukanafun', 'Uruan', 'Urue-Offong/Oruko', 'Uyo'],
  },
  {
    state: 'Anambra',
  ...
  {
    state: 'Yobe',
    lgas: ['Bade', 'Bursari', 'Damaturu', 'Fika', 'Fune', 'Geidam', 'Gujba', 'Gulani', 'Jakusko', 'Karasuwa', 'Machina', 'Nangere', 'Nguru', 'Potiskum', 'Tarmua', 'Yunusari', 'Yusufari'],
  },
  {
    state: 'Zamfara',
...
export const getStateNames = (): string[] => nigeriaLocations.map((loc) => loc.state);

export const getLgasByState = (state: string): string[] | null => {
  const found = nigeriaLocations.find(
    (loc) => loc.state.toLowerCase() === state.toLowerCase(),
  );
  return found ? found.lgas : null;
};

export const isValidState = (state: string): boolean =>
  nigeriaLocations.some((loc) => loc.state.toLowerCase() === state.toLowerCase());

export const isValidLga = (state: string, lga: string): boolean => {
  const lgas = getLgasByState(state);
  return lgas ? lgas.some((l) => l.toLowerCase() === lga.toLowerCase()) : false;
};
